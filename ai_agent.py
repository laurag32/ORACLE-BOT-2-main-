...
from profit_logger import log_profit  # NEW: integrate ProfitLogger

WATCHERS_FILE = "watchers.json"

# Configurable defaults (can be overridden in bot.py/config.json)
    ...
    try:
        ...
        if pid is not None:
            try:
                ...
                pass
        raise ValueError(f"Could not build transaction for {func_name}")
...
def estimate_gas_for_tx(w3: Web3, tx: dict) -> int:
    try:
        ...
        print(f"[AI Agent] Gas estimation failed: {e}; using default {DEFAULT_GAS_ESTIMATE}")
...
def decide_to_harvest(watcher: Dict[str, Any], pending_info: Dict[str, Any], gas_gwei: float, gas_limit: int, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    min_reward_usd = config.get("min_reward_usd", 0.0)  # configurable
    profit_multiplier = config.get("profit_multiplier", 1.05)  # e.g. 5% profit margin
    abs_max_gas = config.get("absolute_max_gas_gwei", 600)

    if gas_gwei > abs_max_gas:
        return {"should": False, "reason": f"gas_above_max ({gas_gwei} gwei)", "expected_profit_usd": reward_usd - gas_cost_usd, "gas_cost_usd": gas_cost_usd}

    if reward_usd < min_reward_usd:
        return {"should": False, "reason": f"reward_below_min (${reward_usd:.4f})", "expected_profit_usd": reward_usd - gas_cost_usd, "gas_cost_usd": gas_cost_usd}

    if reward_usd <= gas_cost_usd:
        return {"should": False, "reason": "gas_cost_exceeds_reward", "expected_profit_usd": reward_usd - gas_cost_usd, "gas_cost_usd": gas_cost_usd}

    expected_profit = reward_usd - gas_cost_usd
    if expected_profit < gas_cost_usd * (profit_multiplier - 1):
        return {"should": False, "reason": f"profit_margin_low (${expected_profit:.4f})", "expected_profit_usd": expected_profit, "gas_cost_usd": gas_cost_usd}
...
def analyze_and_act(w3: Web3, watcher: Dict[str, Any], public_address: str, private_key: str, config: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        print(f"[AI Agent] Error building tx: {e}")
    ...
    print(f"[AI Agent] {watcher.get('name')} pending={pending.get('amount')} {pending.get('symbol')} -> decision={decision}")
    ...
    try:
        ...
        # Log profit
        reward_token = pending.get("symbol", watcher.get("rewardToken", "MATIC"))
        log_profit(watcher.get("name"), pending.get("amount"), reward_token, decision["expected_profit_usd"])

        return tx_hash

    except Exception as e:
        watcher["last_error"] = f"send_tx_failed: {e}"
        print(f"[AI Agent] send_tx failed: {e}")
